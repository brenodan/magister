...
import glob
files = glob.glob("/Users/bdantasc/eclipse-workspace/credential.scan/output/data_splits2/x*.txt")
...
for file in files:
	...
	with open(file) as my_file:
		...
		#print("total number of strings " + str(i) + " : number of candidates " + str(len(my_set)))
	...
	#jac_similarity = -1*np.array([[nltk.jaccard_distance(w1,w2) for w1 in words] for w2 in words])
